...
class qtype_kekule_chem_base_question extends qtype_kekule_multianswer_question {
    ...
    private function _compareMolAnsString($src, $target, $compareMethod, $compareLevel)
    {
        ...
        //If we have choosen the compare arrows exercice
        if ($compareMethod == 13) {
        ...
        if ($this->isBasedOnSmiles($compareMethod))
        {
            ...
            if (!empty($srcSmiles) && !empty($targetSmiles)) {
                if ($compareLevel == qtype_kekule_chem_compare_levels::NON_BONDING_PAIRS){
                    return ((strcmp($srcSmiles, $targetSmiles) == 0)? 1: 0) && qtype_kekule_chem_utils::is_electron_set_the_same_v3($srcDetail,$targetDetail);
                } else {
